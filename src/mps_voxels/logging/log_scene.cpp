...
void DataLog::log<Scene>(const std::string& channel, const Scene& msg)
{
	...
	activeChannels.insert(channel + "/worldFrame");
	std_msgs::String wf; wf.data = msg.worldFrame;
	log(channel + "/worldFrame", wf);
...
bool DataLog::load<Scene>(const std::string& channel, Scene& msg)
{
	...
	std_msgs::String wf;
	load(channel + "/worldFrame", wf);
	msg.worldFrame = wf.data;
