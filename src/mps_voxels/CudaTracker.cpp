...
CudaTracker::CudaTracker(const size_t _buffer,
                         ...
                         : Tracker(_buffer, _options, _track_options)
{

}

void CudaTracker::track()
{
	...
	for (int i = 1; i < static_cast<int>(rgb_buffer.size()) && ros::ok(); ++i)
	{
		...
		for (int p = 0; p < siftData1.numPts; ++p)
		{
			...
			const cudaSift::SiftPoint& spm = siftData2.h_data[sp.match];
			{
				...
				uint16_t dVal1 = depth_buffer[i-1]->image.at<uint16_t>(cv::Point2i(sp.xpos, sp.ypos));
				uint16_t dVal2 = depth_buffer[i]->image.at<uint16_t>(cv::Point2i(spm.xpos, spm.ypos));
