...
void reallySimpleImageMatch()
{
    ...
    std::cerr << "1:4"
              << "\t" << jaccard4.match.first << "\t" << jaccard4.symmetricCover() << std::endl;

    JaccardMatch jaccard3(img1, img3);
    std::cerr << "1:3"
              << "\t" << jaccard3.match.first << "\t" << jaccard3.symmetricCover() << std::endl;

    JaccardMatch jaccard2(img1, img2);
    std::cerr << "1:2"
              << "\t" << jaccard2.match.first << "\t" << jaccard2.symmetricCover() << std::endl;

    JaccardMatch jaccard1(img1, img1);
    std::cerr << "1:1"
              << "\t" << jaccard1.match.first << "\t" << jaccard1.symmetricCover() << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    //	std::string bagFileName = parsePackageURL(testDirName) + ros::this_node::getName() + ".bag";
    std::string bagFileName = parsePackageURL(testDirName) + "2020-06-30/segInfo_0.bag";
    ...
    {
        SubproblemIndex label;
        label.id = pair1.first;
        if (0 == label.id)
        {
            continue;
        }
